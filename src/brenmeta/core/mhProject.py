    ...
    def get_values(self, absolute=True, blend=1.0):
        if absolute:
            if not self.pose_manager:
                raise mhCore.MHError("Cannot get absolute values without pose manager")
...
class ComboPose(object):
    ...
    def get_values(self, summed=True, absolute=True, blend=1.0):
        ...
        if absolute:
            if not self.pose_manager:
                raise mhCore.MHError("Cannot get absolute values without pose manager: {}".format(self))

            values = {}

            for attr, value in summed_deltas.items():
                # if attr not in self.pose_manager.attr_defaults:
                #     raise mhCore.MHError("attr not in defaults: {}".format(attr))
    ...
    def update_name(self, override=True):
        if self.pose.name and not override:
            raise mhCore.MHError("ComboPose is already named: {}".format(self))
    ...
    def deserialize(self, data):  # , pose_manager):
        ...
        #         raise mhCore.MHError("combo pose index out of range: {}".format(data["index"]))
        #
        ...
        if data["input_poses"]:
            ...
            for pose_index in data["input_poses"]:
                if pose_index >= len(self.pose_manager.poses):
                    raise mhCore.MHError("input pose index out of range: {}".format(pose_index))
        ...
        if data["input_combos"]:
            ...
            for pose_index in data["input_combos"]:
                ...
                #     raise mhCore.MHError("input combo index not found: {}".format(pose_index))
                #
...
class Project(object):
    ...
    def reset(self, dna_assets_path):
        ...
        self.input_dna_path = None
        self.output_dna_path = None
        self.bake_config_path = os.path.join(mhCore.DATA_DIR, "configs", "bake_config.json")
    ...
    def read(self, filepath):
        if not os.path.exists(filepath):
            raise mhCore.MHError("File not found: {}".format(filepath))
